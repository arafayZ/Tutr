...
import '../widgets/custom_bottom_nav.dart'; // Import your new separate file
...
class _TutorDashboardState extends State<TutorDashboard> {
  TutorStatus currentStatus = TutorStatus.pending;
  ...
  void _cycleStates() {
    setState(() {
      if (currentStatus == TutorStatus.pending) {
        currentStatus = TutorStatus.approvedEmpty;
      } else if (currentStatus == TutorStatus.approvedEmpty) {
  ...
  Widget build(BuildContext context) {
    final String displayStudents = currentStatus == TutorStatus.active
        ? activeStudents.toString().padLeft(2, '0') : "00";
    final String displayCourses = currentStatus == TutorStatus.active
        ? activeCourses.toString().padLeft(2, '0') : "00";

    return Scaffold(
      backgroundColor: const Color(0xFFF8F9FB),
      floatingActionButton: FloatingActionButton(
        onPressed: _cycleStates,
      ...
      // Calling the externalized Navigation Bar
...
class _ActiveCoursesListView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 15),
      ...mockDbData.map((course) => CourseCard(course: course)).toList(),
    ]);
  }
}
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Center(child: Column(children: [
        ...
        const Text("Nothing Here Yet", style: TextStyle(color: Colors.grey, fontSize: 18)),
      ])),
    ]);
  }
}
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          const _CircleIconButton(icon: Icons.search),
...
class _CircleIconButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Container(padding: const EdgeInsets.all(8), decoration: const BoxDecoration(color: Colors.white12, shape: BoxShape.circle), child: Icon(icon, color: Colors.white, size: 22));
  }
}
